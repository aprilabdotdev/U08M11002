...
\begin{problem}
描述某 LTI 系统的常微分方程为 $y''(t) + 3 y'(t) + 2y(t) = 2f'(t) + 6 f(t)$，已知$y(0^-)=2$，$y'(0^-)=0$，$f(t)=U(t)$，求 $y(0^+)$ 和 $y'(0^+)$。
\quad
\end{problem}
...
\begin{problem}
描述某 LTI 系统的常微分方程为 $y'(t) + 2 y(t) = f''(t) + f'(t) + 2f(t)$，若$f(t)=U(t)$，求该系统的零状态响应。
\quad
\end{problem}
...
\begin{exercise}(1)
	\task 若完全响应为$y(t)=[5e^{-t} + 2e^{-2t}]U(t)$，求该系统的零输入响应和零状态响应；
	\task 若$y(0^-)=10$，求系统的零输入响应；
	\task 若完全响应为$y(t)=[5e^{-t} + 2e^{-2t}]U(t)$，求$y'(t) + y(t) = f(t-2)$的零状态响应；
	\task 若完全响应为$y(t)=[5e^{-t} + 2e^{-2t}]U(t)$，求$y'(t) + y(t) = f'(t) + 2f(t)$的零状态响应。
\end{exercise}
\quad
\end{problem}

\begin{problem}
已知描述系统的常微分方程和起始状态如下，求零输入响应、零状态响应和全响应(从特征根求响应可以查表)。
\begin{exercise}(1)
...
\begin{problem}
如下图所示电路，已知 $R=3\Omega$, $L=1H$, $C=0.5F$, $u_S(t)=\cos t U(t)$, 求 $u_C(t)$ 的零状态响应。
\begin{figure}[H]
...
\begin{problem}
描述某 LTI 系统的常微分方程为 $y''(t) + 5y'(t) + 6y(t) = f(t)$，求冲激响应 $h(t)$。
\quad
\end{problem}
